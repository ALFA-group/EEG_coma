...
def sorting_criterion(s, hospital):
...
def create_patients_files(dataset_path, output_path):
    ...
    for patient in BWH_patients:
        ...
        BWH_patients[patient].sort(key=lambda x: sorting_criterion(x, "BWH"))  # Sort list of files so that they are in temporal order.
    ...
    for patient in BIDMC_patients:
        ...
        BIDMC_patients[patient].sort(key=lambda x: sorting_criterion(x, "BIDMC"))  # Sort list of files so that they are in temporal order.
    ...
    for patient in yale_patients:
        ...
        yale_patients[patient].sort(key=lambda x: sorting_criterion(x, "yale"))  # Sort list of files so that they are in temporal order.
    ...
    for patient in MGH_patients:
        ...
        MGH_patients[patient].sort(key=lambda x: sorting_criterion(x, "MGH"))  # Sort list of files so that they are in temporal order.
